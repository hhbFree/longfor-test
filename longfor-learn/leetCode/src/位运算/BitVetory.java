...
public class BitVetory {
    ...
    public void set(int i){
        ...
        a[P] |= 1 << S;//a[P] |= 1 << S 整句意思是把a[P]元素的第S+1位设置为1，注意从低位到高位设置，即从右到左
    ...
    public void clear(int i){
        ...
        //计算出余数S，利用1左移S位，再取反(~)操作，最后进行与(&)操作，即将a[P]元素的第S+1个(从低位到高位)bit位设置为0，
        // 表示删除该数字
        a[P] &= ~(1 << S);
    ...
    public int get(int i){
        ...
        P = i >> SHIFT;
        S = i &  MASK;
        // System.out.println(a[P] & (1 << S));
        //Integer.bitCount()是返回指定 int 值的二进制补码(计算机数字的二进制表示法都是使用补码表示的)表示形式的 1 位的数量
